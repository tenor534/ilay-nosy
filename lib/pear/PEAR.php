...
// +----------------------------------------------------------------------+
//
// $Id: PEAR.php,v 1.63 2003/06/23 01:27:37 imajes Exp $
//
...
class PEAR
{
    ...
    */
    ...
    {
		if (is_object($data) && (get_class($data) == 'PEAR_Error' || is_subclass_of($data, 'PEAR_Error'))) {
            if (is_null($code)) {
                return true;
                         ...
                         $skipmsg = false)
    {
		...
		if ($skipmsg) {
            $error = new $ec($code, $mode, $options, $userinfo);
        } else {
			$error = new $ec($message, $code, $mode, $options, $userinfo);
		}
		return $error;
    }
