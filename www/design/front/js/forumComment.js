	...
	$("#formButton_ok").click(function(){
		...
		if(document.forumForm.userComment.value.length == 0){
			...
			$.ajax({
				 ...
				 data:"module=forum&action=forumFo_forumAddComment&comment_sujetId="+comment_sujetId+"&comment_utilisateurId="+comment_utilisateurId+"&comment_texte="+comment_texte,
				 dataType:"json",
				 async:false,
				 success:function(datas){
	
					if(datas != '' && datas['commentFor'] != 0)
					{
						var html = '';
						
						html += '<p>'+datas["commentFor"]["texte"]+'</p>';
						html += '<ul>';
						html += '	<p class="regTextPaleNormal borderNoneInline">Publié le: '+datas["commentFor"]["dateCreation"]+'</p>';
