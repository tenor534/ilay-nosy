...
function tmt_validateForm(formNode){
	...
	var initErrorMsg = "Certaines informations sont incomplètes ou invalides, veuillez les compléter correctement.";
...
/*
	...
	infoObj.i = minute;
	infoObj.c = second;	
	infoObj.s = separator1;
	infoObj.t = separator2;
*/
		//Value
		var theValue = fieldNode.value;
		var dateTypes = theValue.split(" ");			
		var dateValue = dateTypes[0];

		// Split the date into 3 different bits using the separator
		//var dateBits = fieldNode.value.split(globalObj.s);
		var dateBits = dateValue.split(globalObj.s);		

		if(dateTypes.length > 1){
			timeValue = dateTypes[1];

			
			var timeBits = timeValue.split(globalObj.t);		
			
			// First try to create a new date out of the bits
			var testDate = new Date();			
			testDate.setFullYear(dateBits[globalObj.y]); 
			testDate.setMonth(dateBits[globalObj.m]-1); 
			testDate.setDate(dateBits[globalObj.d]); 
			testDate.setHours(timeBits[globalObj.h]); 
			testDate.setMinutes(timeBits[globalObj.i]); 
			testDate.setSeconds(timeBits[globalObj.c]); 
			
			var isDate = (testDate.getFullYear() == parseInt(dateBits[globalObj.y]))
					 && (testDate.getMonth() == parseInt(dateBits[globalObj.m]-1))
					 && (testDate.getDate() == parseInt(dateBits[globalObj.d]))
					 && (testDate.getHours() == parseInt(timeBits[globalObj.h]))
					 && (testDate.getMinutes() == parseInt(timeBits[globalObj.i]))
					 && (testDate.getSeconds() == parseInt(timeBits[globalObj.c]));					 
...
//SOLOFO : 14/02/2010 : Deux champ doivent toujours etre égal : cas pour les champs de vérification
tmt_globalRules.twoequal = function(fieldNode){
	var twinNode = document.getElementById(fieldNode.getAttribute("tmt:twoequal"));
			
	if(twinNode.value == fieldNode.value){//les deux champs sont égaux	
		return true;
	}
	return false;
...
tmt_globalDatePatterns["DD-MM-YYYY"] = tmt_dateInfo("^\([0-3][0-9]\)\\-\([0-1][0-9]\)\\-\([0-9]{4}\)$", 2, 1, 0, "-");
tmt_globalDatePatterns["D-M-YYYY"] = tmt_dateInfo("^\([0-3]?[0-9]\)\\-\([0-1]?[0-9]\)\\-\([0-9]{4}\)$", 2, 1, 0, "-");
tmt_globalDatePatterns["DD/MM/YYYY HH:ii:ss"] = tmt_dateTimeInfo("^\([0-3][0-9]\)\/\([0-1][0-9]\)\/\([0-9]{4}\)\ \([0-2][0-9]\)\:\([0-5][0-9]\)\:\([0-5][0-9]\)$", 2, 1, 0, 0, 1, 2, "/", ":");
...
function tmt_dateTimeInfo(rex, year, month, day, hour, minute, second, separator1, separator2){
	var infoObj = new Object;
	infoObj.rex = new RegExp(rex);
	infoObj.y = year;
	infoObj.m = month;
