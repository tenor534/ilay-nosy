...
function doOnLoad(context){
	...
	$($('form',context).get(0)).submit(function(){
		...
		if(tmt_validateForm(this)){
			...
			if($(this).attr('id') == 'packForm') {

			

				if($('#champs_fichier').val() != '') {

					$('#pack_fichier').val($('#champs_fichier').val());
			...
			if($(this).attr('id') == 'actualiteForm') {

			

				if($('#champs_fichier').val() != '') {

					$('#actualite_fichier').val($('#champs_fichier').val());
			...
			if($(this).attr('id') == 'officielForm') {

			

				if($('#champs_fichier').val() != '') {

					$('#officiel_fichier').val($('#champs_fichier').val());
			...
			if($(this).attr('id') == 'banniereForm') {

			

				if($('#champs_fichier').val() != '') {

					$('#banniere_fichier').val($('#champs_fichier').val());
	...
	*/
	...
	$('#btn_supprimer', context).click(function(){
...
//		return confirm("Etes-vous sûr de vouloir archiver ce FAP ?");
	...
	*/
	...
	$("a[@alt=Drapeau]").click(function(){

		if(confirm("Voulez-vous changer de langue?")){

			

			//Id de la langue choisie

			var idLangue = $(this).attr('id');
			...
			$.ajax({
				 ...
				 data:"module=session&action=sessionBo_updateSessionUser&idLangue=" + idLangue,
...
//	$('#errorMessage').html('Certaines informations sont incomplètes ou invalides, veuillez les compléter correctement.');
	...
	$('#errorMessage').html('Some information is incomplete or invalid, please fill properly.');	
