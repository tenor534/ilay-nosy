...
window.onbeforeunload = function(e){
	...
	if((g_supprvisuel && $('#actualite_id').val()=="") || ($('input[@name^=fichierasuppr_b]').length>0)){
		suppr='';
		$('input[@name^=fichierasuppr_b]').each(function(index){
			if($(this).val().length>0){
				if($(this).val().toLowerCase()!=$('#actualite_photo').val().toLowerCase())
					suppr+=$(this).val();
		...
		$.ajax({
			 ...
			 data:"module=actualite&action=actualiteBo_traitementVisuels&fl=p&process=suppr&fichier="+suppr,
			 dataType:"json",
			 async:false
		});
	}
...
//		alert(this.name);
	...
	if($('#actualite_photo').val()!=""){
		extension=$('#actualite_photo').val().split(['.']);
		...
		form=document.getElementById('actualiteForm');
	...
	$('#champsvisuel').bind("focus",function() {
		...
		$.ajax({
			 type:"POST",
			 url:j_basepath+'index.php',
			 data:"module=actualite&action=actualiteBo_traitementVisuels&fl=p&process=resize&fichier="+$(this).val(),
			 dataType:"json",
			 async:false,
			 success:function(resultat){
				$('#appercuPhoto').empty();
				$('#appercuPhoto').html(resultat.visuel);
				$('#auxvisuel').val(j_basepath+j_actualite_resize + 'photos/' +  resultat.image);
				...
				if($('#auxvisuel').val().length>0)
					$('#actualite_photo').val($('#auxvisuel').val());

				input_h=document.createElement('input');
				input_h.type='hidden';
				$(input_h).attr({'name':'fichierasuppr_'+$('input[@name^=fichierasuppr_]').length,'value':j_basepath + j_actualite_resize + 'photos/'  + resultat.image});
				$('#actualiteForm').append($(input_h));
				form=document.getElementById('actualiteForm');
				form.tmt_validator = new tmt_formValidator(form);
			 }
		});
	...
	$("#profile_picture_remove").click(function(){
		...
		$.ajax({
			 type:"POST",
			 url:j_basepath+'index.php',
			 data:"module=actualite&action=actualiteBo_removePhoto&photo_id="+photo_id+"&photo_photo="+photo_photo,
			 dataType:"json",
			 async:false,
			 success:function(resultat){														 	
				$("#profile_pic").attr("src",""+j_basepath+"resize/actualite/images/popup/noPhoto.jpg");
				$("#profile_pic_popup").attr("src",""+j_basepath+"resize/actualite/images/popup/noPhoto.jpg");				
				//$("#imgthumb"+photo_id).attr("src",""+j_basepath+"resize/actualite/images/abrege/noPhoto.jpg");				
				$("#linkthumb"+photo_id).attr("href","javascript:voirImg('noPhoto.jpg', "+photo_id+");");					
				
				$("#linkthumb"+photo_id).html("");
				zHtml = '<img width="90" height="68" border="0" src="/resize/actualite/images/abrege/noPhoto.jpg" alt="noPhoto.jpg" name="imgthumb' + photo_id + '" id="imgthumb' + photo_id + '" onmouseover="voirImg(\'noPhoto.jpg\', ' + photo_id + ');">';
	...
	$("#user_photo").change(function(){
		...
		var options = {
			 type:"POST",
			 url:j_basepath+'index.php?module=actualite&action=actualiteBo_changePhoto',
			 success: function(msg) {
				...
				$("#profile_pic").attr("src",""+j_basepath+"resize/actualite/images/popup/" + src);
				$("#profile_pic_popup").attr("src",""+j_basepath+"resize/actualite/images/popup/" + src);				
				$("#linkthumb"+photo_id).attr("href","javascript:voirImg('" + src + "', " + photo_id + ");");					

				//$("#imgthumb"+photo_id).attr("src",""+j_basepath+"resize/actualite/images/abrege/" + src);				
				//$("#imgthumb"+photo_id).attr("onmouseover","voirImg('" + src + "', " + photo_id + ");");					
				
				$("#linkthumb"+photo_id).html("");
				zHtml = '<img width="90" height="68" border="0" src="'+j_basepath+'resize/actualite/images/abrege/' + src + '" alt="' + src + '" name="imgthumb' + photo_id + '" id="imgthumb' + photo_id + '" onmouseover="voirImg(\'' + src + '\', ' + photo_id + ');">';
				$("#linkthumb"+photo_id).append(zHtml);
...
function clickAction(obj){
	...
	if(id == 'photo')
	{
		ServerPath = j_basepath + j_actualite_medias + 'photos/';
		champs = 'champsvisuel';
	}
	if(id == 'fichier')
	{
		ServerPath = j_basepath + j_actualite_medias + 'fichiers/';
