...
(function($) {    
	...
	$.fn.tableSorter = function(o) {
		...
		return this.each(function(){
			...
			//Sylvain261
			function isColumnDisabled(columnIndex){
				if(!defaults.disabledColumn){
					return false;
				}
				for(iDis=0;iDis<defaults.disabledColumn.length;iDis++){
					if(defaults.disabledColumn[iDis] == columnIndex){
						return true;
					}
				}
				return false;
			...
			function buildColumnHeaders() {
				...
				for( var i=0; i < COLUMN_HEADER_LENGTH; i++ ) {
					...
					if(!isColumnDisabled(i) &&  oDataSampleRow && !$.tableSorter.utils.isHeaderDisabled(defaults,oCell,defaults.disableHeader,i)) {
						...
						$(oCell).addClass(defaults.headerDisabledClass);
			...
			function sortOnColumn(oCell,dir,index) {
				...
				/** 
				* to do Tojo Randrianantenaina 
				* 
				**/ 

				/** recupération de l'attribut colonne de la table **/
				sortField = $(oTable).attr('colonne');
				oCells = new Array();
				rowHead = oTable.rows[0];
				for(i=0;i<sortField;i++){
				   cp = $(rowHead.cells[i]).attr("colspan");
				   if(cp!=undefined){
				   	rowHead = oTable.rows[1];
						for(j=0;j<cp;j++,i++){// prise des cells pour les colspan
							oCells[i] = rowHead.cells[j];
						}
						i--;
				   	rowHead = oTable.rows[0];
					} else {
						oCells[i] = rowHead.cells[i];
					}
				}
				for (i=0;i<sortField;i++){
// 					alert(oCell.innerHTML);
					...
					/** on enlève la classe "sortable" (classe contenant les flèches haut et bas)*/
					$("thead tr",oTable).find("th:eq(" + COLUMN_INDEX + ")").removeClass(defaults.headerClass);
					
					/** on ajoute la classe "sortUp" ou "sortDown" selon la valeur du dir */
					
					$("thead tr",oTable).find("th:eq(" + COLUMN_INDEX + ")").addClass((dir % 2 ? defaults.sortClassAsc : defaults.sortClassDesc));
					
					} else{
						/**  sinon mettre tous les colonnes avec la classe "sortable" */
						
						$("thead tr",oTable).find("th:eq(" + i + ")").addClass(defaults.headerClass);
					}
