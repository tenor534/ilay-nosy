...
function voirImg (imgSrc, id){
	$("#profile_pic").attr("src",""+j_basepath+"resize/banniere/images/popup/" + imgSrc);
	$("#picture_id").val(id);
	$("#photo_id").val(id);
...
window.onbeforeunload = function(e){
	...
	if((g_supprvisuel && $('#banniere_id').val()=="") || ($('input[@name^=fichierasuppr_b]').length>0)){
		suppr='';
		$('input[@name^=fichierasuppr_b]').each(function(index){
			if($(this).val().length>0){
				if($(this).val().toLowerCase()!=$('#banniere_photo').val().toLowerCase())
					suppr+=$(this).val();
		...
		$.ajax({
			 ...
			 data:"module=banniere&action=banniereBo_traitementVisuels&fl=p&process=suppr&fichier="+suppr,
			 dataType:"json",
			 async:false
		});
	}
...
//		alert(this.name);
	...
	if($('#banniere_photo').val()!=""){
		extension=$('#banniere_photo').val().split(['.']);
		...
		form=document.getElementById('banniereForm');
	...
	$('#champsvisuel').bind("focus",function() {
		...
		$.ajax({
			 type:"POST",
			 url:j_basepath+'index.php',
			 data:"module=banniere&action=banniereBo_traitementVisuels&fl=p&process=resize&fichier="+$(this).val(),
			 dataType:"json",
			 async:false,
			 success:function(resultat){
				$('#appercuPhoto').empty();
				$('#appercuPhoto').html(resultat.visuel);
				$('#auxvisuel').val(j_basepath+j_banniere_resize + 'photos/' +  resultat.image);
				extension=resultat.image.split(['.']);
				switch(extension[extension.length-1].toLowerCase()){
					case 'swf':
						if($('.clearfix').length>1)
						$('.clearfix').gt(0).hide();
						//$('#banniere_url').removeAttr('tmt:required');
						break;
					default:
						$('.clearfix').each(function(){
							$(this).css('display','block');
				...
				//$('#banniere_photo').val(resultat.image);
				if($('#auxvisuel').val().length>0)
					$('#banniere_photo').val($('#auxvisuel').val());

				input_h=document.createElement('input');
				input_h.type='hidden';
				$(input_h).attr({'name':'fichierasuppr_'+$('input[@name^=fichierasuppr_]').length,'value':j_basepath + j_banniere_resize + 'photos/'  + resultat.image});
				$('#banniereForm').append($(input_h));
				form=document.getElementById('banniereForm');
				form.tmt_validator = new tmt_formValidator(form);
			 }
		});
	...
	$("#profile_picture_remove").click(function(){
		...
		$.ajax({
			 type:"POST",
			 url:j_basepath+'index.php',
			 data:"module=banniere&action=banniereBo_removePhoto&photo_id="+photo_id+"&photo_photo="+photo_photo,
			 dataType:"json",
			 async:false,
			 success:function(resultat){														 	
				$("#profile_pic").attr("src",""+j_basepath+"resize/banniere/images/popup/noPhoto.jpg");
				$("#profile_pic_popup").attr("src",""+j_basepath+"resize/banniere/images/popup/noPhoto.jpg");				
				//$("#imgthumb"+photo_id).attr("src",""+j_basepath+"resize/banniere/images/abrege/noPhoto.jpg");				
				$("#linkthumb"+photo_id).attr("href","javascript:voirImg('noPhoto.jpg', "+photo_id+");");					
				
				$("#linkthumb"+photo_id).html("");
				zHtml = '<img width="90" height="68" border="0" src="/resize/banniere/images/abrege/noPhoto.jpg" alt="noPhoto.jpg" name="imgthumb' + photo_id + '" id="imgthumb' + photo_id + '" onmouseover="voirImg(\'noPhoto.jpg\', ' + photo_id + ');">';
	...
	$("#user_photo").change(function(){
		...
		var options = {
			 type:"POST",
			 url:j_basepath+'index.php?module=banniere&action=banniereBo_changePhoto',
			 success: function(msg) {
				...
				$("#profile_pic").attr("src",""+j_basepath+"resize/banniere/images/popup/" + src);
				$("#profile_pic_popup").attr("src",""+j_basepath+"resize/banniere/images/popup/" + src);				
				$("#linkthumb"+photo_id).attr("href","javascript:voirImg('" + src + "', " + photo_id + ");");					

				//$("#imgthumb"+photo_id).attr("src",""+j_basepath+"resize/banniere/images/abrege/" + src);				
				//$("#imgthumb"+photo_id).attr("onmouseover","voirImg('" + src + "', " + photo_id + ");");					
				
				$("#linkthumb"+photo_id).html("");
				zHtml = '<img width="90" height="68" border="0" src="'+j_basepath+'resize/banniere/images/abrege/' + src + '" alt="' + src + '" name="imgthumb' + photo_id + '" id="imgthumb' + photo_id + '" onmouseover="voirImg(\'' + src + '\', ' + photo_id + ');">';
				$("#linkthumb"+photo_id).append(zHtml);
...
function clickAction(obj){
	...
	if(id == 'photo')
	{
		ServerPath = j_basepath + j_banniere_medias + 'photos/';
		champs = 'champsvisuel';
	}
	if(id == 'fichier')
	{
		ServerPath = j_basepath + j_banniere_medias + 'fichiers/';
