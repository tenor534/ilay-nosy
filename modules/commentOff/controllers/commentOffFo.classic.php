...
class utilisateurFoCtrl extends jController {
	...
	/**
    * Supprime la commentOff de profil d'un membre utilisateur
	* 
    ...
    */
    function removeCommentOff() {
		...
		//Nom du fichier
		$zNomFichier = $utilisateur->profilCommentOff;
		...
		$utilisateur->commentOff = "";
		
		//Update
		$iReturn = utilisateurSrv::sauvegardeMembre($utilisateur);

		//Paramètres
		$response['imgSrc'] = "nocommentOff.jpg";
	...
	/**
    * Changement de la commentOff de profil d'un membre utilisateur
	* 
	* Utilisé via mode edition popup 
	* Recoit en paramètre l'ensemble des données du formulaire d'édition d'un membre
	* Une fois ls utilisateur sauvegardée, redirige vers la page de liste des membres
    */
    function changeCommentOff() {
		//Préparation de la réponse
		global $gJConfig;
		...
		//upload du fichier
		$zNomFichier 		= tools::uploadFile ("user_commentOff", $zPathMedia) ;
		$utilisateur->commentOff = $zNomFichier;
		...
		if($zNomFichier){
			...
			//resize fichier au format commentOff : 180 x 135
			//tools::resizeImage($zPathMedia.''.$zNomFichier, $zPathResize.'commentOff/'.$zNomFichier, ANNONCE_PHOTO_WIDTH, ANNONCE_PHOTO_HEIGHT, 'ratio');
			tools::resizeImage2($zPathMedia.''.$zNomFichier, $zPathResize.'commentOff/'.$zNomFichier, $zPathResize.'commentOff/blank.jpg', ANNONCE_PHOTO_WIDTH, ANNONCE_PHOTO_HEIGHT, $im_color, $im_cropratio, $im_quality);
