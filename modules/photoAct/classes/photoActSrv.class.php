...
class photoActSrv {
    ...
    */
    static function chargeListePhoto($sortField="photo_photo", $sortDirection="ASC",$iDebutList=0 , $iListAll = 0) {
		...
		//$zSql  = 	" SELECT s.* FROM photoAct AS s";
		
		$zSql  = 	" SELECT p.* ";
		$zSql .=	" ,a.actualite_reference ";
		
		$zSql .=	" FROM photoAct AS p " ;
		...
		//tot enregistrement
		$rsCount = $cnx->query("SELECT COUNT(*) as nbEnreg FROM photoAct");
    ...
    */
    static function chargePhoto($photoId) {
		...
		$zQuery = "SELECT photo_id
			...
			FROM photoAct WHERE photo_id=".$photoId;
		//echo $zQuery;	
    ...
    */
    static function sauvegardePhoto($photo) {
		...
		if (!isset($photo->id) || $photo->id==0) { // insertion
			...
			//Requette d'ajout
			$zQuery = "INSERT INTO photoAct VALUES (
				'0'
				," .$photo->actualiteId ."
			...
			$zQuery = "UPDATE photoAct SET \n
					photo_id=".$oCnx->quote($photo->id)."";
					
			if (isset($photo->actualiteId)) {
				$zQuery .= "\n, photo_actualiteId=".$photo->actualiteId."";
			}
    ...
    */
	static function supprimePhoto($photoId) {
		...
		$zQuery = "DELETE FROM photoAct WHERE photo_id=$photoId";
		$rConn->exec($zQuery);
    ...
    */
	...
	{
		$oCnx = jDb::getConnection();
		$zQuery ="UPDATE photoAct SET photo_statut ='".$statut."' WHERE photo_id =".$idPhoto;
		try {
    ...
    */
    static function getDaoPhoto() {
		...
		$object = jDao::createRecord("photoAct~photo");
	...
	*/
	static function infosPhoto($photoId){
		$dao=jDao::create("photoAct~photo");
		if(!($photo=$dao->get($photoId))){
			$dao=new jSelectorDao('photoAct~photo','');
	...
	*/
	static function getAllPhoto($idActualiteId)
	{
		$zQuery = "SELECT r.*
				FROM photoAct AS r 
				WHERE r.photo_actualiteId = '".$idActualiteId."' 
				ORDER BY r.photo_id";
      	$pDbw = jDb::getDbWidget();
	...
	*/
	static function getAllPhotoActive($idAnnonceId)
	{
		$zQuery = "SELECT r.*
				FROM photoAct AS r 
				WHERE r.photo_actualiteId = '".$idAnnonceId."' 
	...
	* @return tableau de boissons
	*/
	static function getAllPhotoAct($idActualiteId)
	{
		$zQuery = " SELECT p.*
					FROM photoAct AS p 
					WHERE p.photo_actualiteId = '".$idActualiteId."' 					
					AND p.photo_photo <> 'noPhoto.jpg' 					
					ORDER BY p.photo_id ASC";
      	$pDbw = jDb::getDbWidget();
      	$toPhotoAct = $pDbw->fetchAll($zQuery);
		
		$tResult = array () ;
		foreach($toPhotoAct as $oPhotoAct)
		{
			$oPhotoAct->photo_photo = stripslashes($oPhotoAct->photo_photo);
			array_push($tResult, $oPhotoAct);
		}
		return $tResult;
	}	
