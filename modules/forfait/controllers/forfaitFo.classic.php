...
class forfaitFoCtrl extends jController {
	...
	/**
    * Supprime la photo de profil d'un membre forfait
	* 
	* Utilisé via mode edition popup 
	* Recoit en paramètre l'ensemble des données du formulaire d'édition d'un membre
	* Une fois ls forfait sauvegardée, redirige vers la page de liste des membres
    */
    function removePhoto() {
		...
		$forfait = jMagicLoader::arrayToObject($this->request->params, 'user');
		
		//Nom du fichier
		$zNomFichier = $forfait->profilPhoto;
		...
		//Update
		$iReturn = forfaitSrv::sauvegardeMembre($forfait);
	...
	/**
    * Changement de la photo de profil d'un membre forfait
	* 
	* Utilisé via mode edition popup 
	* Recoit en paramètre l'ensemble des données du formulaire d'édition d'un membre
	* Une fois ls forfait sauvegardée, redirige vers la page de liste des membres
    */
    function changePhoto() {
		...
		//$zPath = JELIX_APP_WWW_PATH.'medias/forfait/' ;		
	...
	/**
    * Enregistrement des données d'un membre forfait
	* 
    ...
    */
    function sauvegardeMembre() {
		...
		$forfait->paysId 			= $forfait->pays;
		$forfait->profilId 			= $forfait->profil;
		$forfait->dateNaissance 	= tools::toDateSQL($forfait->dateNaissance);

		//$forfait->id 				= 0;
		//$forfait->login 			= "";
		
		if($forfait->id != 0){
			$forfait->dateModification 	= date("y-m-d"); 
		}else{
			$forfait->dateModification 	= ""; 
			$forfait->dateCreation 		= date("y-m-d");		
		}
		$forfait->statut 			= 1; //3 = en attente de confirmation
