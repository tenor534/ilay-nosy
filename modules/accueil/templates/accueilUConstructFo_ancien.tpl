	<!-- Zone haut publicitaire -->    
    <div id="mastAdvertWrap">  
    	{$logo}  
    	{*$mastAdvert*}  
    </div><!--  mastAdvertWrap: [end] -->    
    
	<div id="bodyWrap">    
   		<div id="wrapper" class="home">
			<div id="mastHead">
		    	{$underConstWrap}  
				<div id="mainZoneRight">
                	{*$searchNav*}
                    {*$mainNavWrap*}
                </div>
			</div><!-- mastHead: [end] -->
			
			<div id="neapolitan">
				<div id="neapolitanTop">
                	<img src="{$j_basepath}design/front/images/v5/cornerTopLeft.gif" alt="" />
                </div><!--neapolitanTop: [end] -->
				<div id="main" class="fullWhite">
                	<div id="white">                    	
						<!--h1>actualit&eacute;s internationales </h1-->
                        <div id="leftColumn">
	                    	{$leftPageAdsTop}
                        	{*$leftPageContactTop*}
                        	{*$leftPageHowTop*}
							{*$homePageNew*}
                            {*$homePageAnnonceRencontre*}
							{*$homePageCulture*}
							{*$homePageForum*}
	                    	{$leftPageAds}                            
                        </div><!-- leftColumn:[end] -->
                        
                        <div id="middleContent">
	                        								
								<div id="mainNews" class="row">
                                    <div class="header">
                                        <h3 class="h3_autres">Web site - UnderConstruction... !</h3>
                                    </div><!-- header:[end] -->
                                                                                                                                                         
                                    <div class="singleColumnBig">
                                        <a target="_blank" title="Web site - UnderConstruction... !" href="#">
                                            <h2>Web site - UnderConstruction... !</h2>
                                        </a>
                                        <a target="_blank" title="Web site - UnderConstruction... !" href="#">
                                          <img alt="" src="{$j_basepath}design/front/images/v6/underconstruction.jpg" height="235" width="469">
                                        </a>
                                        <span class="viewsComments" style="height:5px;">
                                        </span>
                                    </div><!-- :[end] -->
                                </div> 

	                        {$contentPageMainAdverts}
	                        {*$contentPageMainAnnounces*}
                        	{*st by : $contentPageMainNews*}
                        	{*st by : $contentPageReelFilNews*}                            
                        </div><!-- middleColumn:[end] -->                                                                      
                	</div><!-- end white -->  
                    <div id="brown">
                    
                    	    {*$innerPageAdSpace*}            
                         	{*$innerPagePetiteAnnonce*}
                           	{*$innerPageNew*}                          
						<div id="adRightFo">
                           	{*$innerPageAnnonceVehicule*}
                           	{*$innerPageAnnonceImmobilier*}
                           	{*$innerPageAnnonceEmploi*}
                           	{*$innerPageAnnonceAutres*}
                            {*$innerPageProjet*}                                
                            {*$innerPagePratique*}                                
                        </div><!--adRightFo: [end]-->                            
                    	{$innerPageAdSpaces}
                    </div><!--end brown -->   
                </div>
                {*$bootFoot*}        
				{$underConstFoot}        