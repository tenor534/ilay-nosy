...
{else}	  
	...
	<div class="ajaxZone">
		<form enctype="multipart/form-data" id='utilisateurForm' name='utilisateurForm' action="{jurl 'utilisateur~utilisateurBo_sauvegardeUtilisateur', array('page'=>$page)}" method="POST" tmt:validate="true" tmt:callback="displayError">
		  ...
		  <p class="clearfix">
			<label>CommentFor : </label>
			<span class="champ">
				<img id="profile_pic" name="profile_pic" src="{$j_basepath}resize/utilisateur/images/detail/{if $utilisateur->utilisateur_commentFor != ""}{$utilisateur->utilisateur_commentFor}{else}nocommentFor.jpg{/if}" alt="{$utilisateur->utilisateur_prenom} {$utilisateur->utilisateur_nom}">	    
			</span>
		  </p>
		  <p class="clearfix">
			<label>Profile : *</label>
