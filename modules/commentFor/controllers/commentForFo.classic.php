...
class utilisateurFoCtrl extends jController {
	...
	/**
    * Supprime la commentFor de profil d'un membre utilisateur
	* 
    ...
    */
    function removeCommentFor() {
		...
		//Nom du fichier
		$zNomFichier = $utilisateur->profilCommentFor;
		...
		$utilisateur->commentFor = "";
		
		//Update
		$iReturn = utilisateurSrv::sauvegardeMembre($utilisateur);

		//Paramètres
		$response['imgSrc'] = "nocommentFor.jpg";
	...
	/**
    * Changement de la commentFor de profil d'un membre utilisateur
	* 
	* Utilisé via mode edition popup 
	* Recoit en paramètre l'ensemble des données du formulaire d'édition d'un membre
	* Une fois ls utilisateur sauvegardée, redirige vers la page de liste des membres
    */
    function changeCommentFor() {
		//Préparation de la réponse
		global $gJConfig;
		...
		//upload du fichier
		$zNomFichier 		= tools::uploadFile ("user_commentFor", $zPathMedia) ;
		$utilisateur->commentFor = $zNomFichier;
		...
		if($zNomFichier){
			...
			//resize fichier au format commentFor : 180 x 135
			//tools::resizeImage($zPathMedia.''.$zNomFichier, $zPathResize.'commentFor/'.$zNomFichier, ANNONCE_PHOTO_WIDTH, ANNONCE_PHOTO_HEIGHT, 'ratio');
			tools::resizeImage2($zPathMedia.''.$zNomFichier, $zPathResize.'commentFor/'.$zNomFichier, $zPathResize.'commentFor/blank.jpg', ANNONCE_PHOTO_WIDTH, ANNONCE_PHOTO_HEIGHT, $im_color, $im_cropratio, $im_quality);
