...
class utilisateurFoCtrl extends jController {
	...
	/**
    * Supprime la commentAct de profil d'un membre utilisateur
	* 
    ...
    */
    function removeCommentAct() {
		...
		//Nom du fichier
		$zNomFichier = $utilisateur->profilCommentAct;
		...
		$utilisateur->commentAct = "";
		
		//Update
		$iReturn = utilisateurSrv::sauvegardeMembre($utilisateur);

		//Paramètres
		$response['imgSrc'] = "nocommentAct.jpg";
	...
	/**
    * Changement de la commentAct de profil d'un membre utilisateur
	* 
	* Utilisé via mode edition popup 
	* Recoit en paramètre l'ensemble des données du formulaire d'édition d'un membre
	* Une fois ls utilisateur sauvegardée, redirige vers la page de liste des membres
    */
    function changeCommentAct() {
		//Préparation de la réponse
		global $gJConfig;
		...
		//upload du fichier
		$zNomFichier 		= tools::uploadFile ("user_commentAct", $zPathMedia) ;
		$utilisateur->commentAct = $zNomFichier;
		...
		if($zNomFichier){
			...
			//resize fichier au format commentAct : 180 x 135
			//tools::resizeImage($zPathMedia.''.$zNomFichier, $zPathResize.'commentAct/'.$zNomFichier, ANNONCE_PHOTO_WIDTH, ANNONCE_PHOTO_HEIGHT, 'ratio');
			tools::resizeImage2($zPathMedia.''.$zNomFichier, $zPathResize.'commentAct/'.$zNomFichier, $zPathResize.'commentAct/blank.jpg', ANNONCE_PHOTO_WIDTH, ANNONCE_PHOTO_HEIGHT, $im_color, $im_cropratio, $im_quality);
