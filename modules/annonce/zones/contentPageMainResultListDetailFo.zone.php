...
class contentPageMainResultListDetailFoZone extends jZone {
	...
	*/
    protected $_tplname='annonce~contentPageMainResultListDetailFo.zone';
	...
	*/
	protected function _prepareTpl(){
		...
		$tParams = array('zone'=> $this->getParam('zone','annonce~contentPageMainResultListDetailFo'), 'nbPagination'=> $this->nbPagination, 'affichage'=> $this->affichage, 'cid'=> $this->cid, 'rid'=> $this->rid, 'mot'=> $this->mot, 'crid'=> $this->crid, 'parution'=> $this->parution, 'province'=> $this->province, 'localite'=> $this->localite, 'prix1'=> $this->prix1, 'prix2'=> $this->prix2);
		...
		foreach($tResult['listeAnnonce'] as $zKey => $oAnnonce){
			...
			$toPack 		= packSrv::chargePack($toForfait->forfait_packId);
			$toLocalite		= annonceSrv::getLocalite($oAnnonce->annonce_localiteId);
			$toProvince		= annonceSrv::getProvince($toLocalite->localite_provinceId);

			$oAnnonce->annonce_pack 	= $toPack->pack_libelle;
			$oAnnonce->annonce_localite = $toLocalite->localite_code . " " . $toLocalite->localite_libelle;
			$oAnnonce->annonce_province = $toProvince->province_libelle;
			...
			switch( $oAnnonce->annonce_offreId ){
				case 1 : 
					$oAnnonce->annonce_offre = "particulier";
					break;
				case 2 : 
					$oAnnonce->annonce_offre = "professionnel";
					break;
				case 3 : 
					$oAnnonce->annonce_offre = "commercial";
					break;
				default:	
					$oAnnonce->annonce_offre = "candidat";
					break;
			}		
		...
		$tHead = array(
					...
					,array('sortField'=> "annonce_parution", 'libelle'=> "Parution", 'modifier'=> '', 'align'=> "_center", 'class'=> 'annonceHead5')									
					);	
