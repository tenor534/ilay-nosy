...
/**
* Zone affichant le bloc officiel "A la Une" de gauche en FO, triées par date de publication anti-chronologique 
*
* @package ilay-nosy
* @subpackage officiel
*/
class homePageNewFoZone extends jZone {
    ...
    protected $_tplname='officiel~homePageNewFo.zone';
	...
	*/
	protected function _prepareTpl(){
		...
		//CATEGORIES
		$toCategResults = categorieOffSrv::chargeAllCategorieOff();
		foreach ($toCategResults as $oResults){
			$tCategs[$oResults->categorieOff_id] = 0;
		}
		
		//OFFICIELS
		$toResults  		= officielSrv::getTopActLaUNE(10);		
		foreach ($toResults as $oResults){
			...
			//Inc nbAct per Categ
			$tCategs[$oResults->officiel_categorieOffId]++;
			array_push( $toOfficiels, $oResults);
		}

		//CATEGORIES
		foreach ($toCategResults as $oResults){
			if($tCategs[$oResults->categorieOff_id] > 0){			
				array_push( $toCategorieOffs, $oResults);
			}	
		}
