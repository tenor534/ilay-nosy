...
class actualiteBoCtrl extends jController {
    ...
    */
    function sauvegardeActualite() {
		...
		//Save				
		$actualite->photo			= substr($actualite->photo,strlen($gJConfig->urlengine['basePath'].ACTUALITE_PATH_RESIZE."photos/"),strlen($actualite->photo));
		$actualite->photoSave		= substr($actualite->photoSave,strlen($gJConfig->urlengine['basePath'].ACTUALITE_PATH_RESIZE."photos/"),strlen($actualite->photoSave));
...
//-----------------------------------------------------------------------
if($actualite->photo != $actualite->photoSave){

	//Nom du fichier
	$zNomFichier = $actualite->photo;
	
	//Répertoire d'upload sur le serveur
	$zPathMedia = ACTUALITE_PATH_RESIZE."photos/";		
