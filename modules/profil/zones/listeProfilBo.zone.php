...
/**
* Zone affichant la liste des profils
*
* @package ilay-nosy
* @subpackage profil
*/
class listeProfilBoZone extends jZone {
 
	/**
	* Nom du template HTML
	*/
	protected $_tplname='profil~listeProfilBo.zone';

	/**
	* Tri par défaut de la liste
	*/
	public $sortField = 'profil_libelle';
	...
	*/
	protected function _prepareTpl(){
		...
		$tParams = array('zone'=> $this->getParam('zone','profil~listeProfilBo'));
		...
		$tResult = array('iNbEnreg'=>0, 'listeProfil'=>array());

		$tResult = profilSrv::chargeListeProfil($this->sortField, $this->sortDirection,$iDebutListe);	
	
		$nbPage = ceil($tResult['iNbEnreg'] / PAGINATION_NBITEMPARPAGE) ;		

		$tHead = array(array('sortField'=> "profil_libelle", 'libelle'=> "Libellé", 'modifier'=> '', 'align'=> "_left")
				,array('sortField'=> "profil_code", 'libelle'=> "Code", 'modifier'=> '', 'align'=> "_center")
		);
		//print_r ($oNavBar->tiPages) ;
		//Affichage
		$this->_tpl->assign('i', 0);
		$this->_tpl->assign('nbPage', $nbPage);
		$this->_tpl->assign('toProfil', $tResult['listeProfil']);
