...
class utilisateurBoCtrl extends jController {
	...
	public $pluginParams = array(
		...
		'unicityEmail'=>array('auth.required'=>false)	
	);
	...
	* 
	* Recoit le login
	* Une fois ls utilisateur supprimée, redirige vers la page de liste des utilisateurs
    */
    function unicityLogin() {
		...
		//Récupération des paramètres
		$utilisateur_login = $this->param('utilisateur_login','', true);
		if ($utilisateur_login == '') {
			throw new Exception('Invalid parameter utilisateur_login');
		}

		//Suppression
		jClasses::inc('utilisateurSrv');
		$result['foundUser'] = utilisateurSrv::unicityUtilisateurLogin($utilisateur_login);		
		$rep->datas=$result;
    ...
    */
    function unicityEmail() {
		...
		//Récupération des paramètres
		$utilisateur_email = $this->param('utilisateur_email','', true);
		if ($utilisateur_email == '') {
			throw new Exception('Invalid parameter utilisateur_email');
		}

		//Suppression
		jClasses::inc('utilisateurSrv');
		$result['foundUser'] = utilisateurSrv::unicityUtilisateurEmail($utilisateur_email);		
		$rep->datas=$result;
